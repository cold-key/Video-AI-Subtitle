...
可在自己的服务器或 NAS 部署私有共享缓存。在 **设置 → 更多设置 → 共享缓存** 填写服务地址和令牌并启用；已有完整结果可通过 **导入已有缓存** 上传。另一台电脑处理同一视频时直接复用字幕、译文和摘要，正常并发时只有一台电脑处理，其余等待结果。

此功能默认关闭。共享服务只保存字幕结果及断点，不接收音频、Cookie 或模型 API Key。断线且本机无结果时默认等待，可手动选择“仅本机处理”（可能重复消耗模型额度）。[部署、备份和完整使用说明](shared_cache/README.md)。
...
B 站视频跳转后若字幕时间仍有偏差，可打开播放器侧栏的 **更多工具 → 字幕时间两点校准**。在两个相距较远、能够确认实际语句的位置，分别暂停在语句开头，输入字幕列表中对应句子的起始时间并记录。两点校准会按线性时间映射修正字幕显示和点击字幕跳转；校准按视频分 P 保存在本机，可随时清除。
...
- **播放界面：** 可调字幕语言、字号、位置、背景，校准 B 站字幕时间，及侧栏位置、宽度、布局和透明度。
